...
import { Inbox, Clock, XCircle, ShieldCheck, Building, UserCheck } from "lucide-react";
...
export default function ApplicationList({
  ...
  if (applications.length === 0) {
    return <div className="text-center py-12 text-muted-foreground">No applications found.</div>;
  }
          ...
          <Card className="group h-full rounded-3xl border-border/50 bg-background/40 backdrop-blur-xl hover:shadow-2xl hover:shadow-primary/5 transition-all duration-500 overflow-hidden relative">
            ...
            <CardContent className="p-6 pt-0 space-y-4">
              ...
              <div className="flex flex-wrap gap-2 pt-2 border-t border-border/50">
                <Badge
                  variant="outline"
                  className={`text-[9px] uppercase font-bold py-0 h-5 ${app.facultyApproved ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20" : "bg-muted text-muted-foreground border-border"}`}
                >
                  Faculty: {app.facultyApproved ? "OK" : "..."}
                </Badge>
                <Badge
                  variant="outline"
                  className={`text-[9px] uppercase font-bold py-0 h-5 ${app.industryApproved ? "bg-blue-500/10 text-blue-600 border-blue-500/20" : "bg-muted text-muted-foreground border-border"}`}
                >
                  Partner: {app.industryApproved ? "OK" : "..."}
                </Badge>
                <Badge
                  variant="outline"
                  className={`text-[9px] uppercase font-bold py-0 h-5 ${app.adminApproved ? "bg-purple-500/10 text-purple-600 border-purple-500/20" : "bg-muted text-muted-foreground border-border"}`}
                >
                ...
                {app.status === "PENDING" && (
                  <div className="flex gap-2">
                    ...
                    {(currentUserRole === "SUPERADMIN" ||
                      ...
                      (currentUserRole === "INDUSTRY_PARTNER" &&
                        app.project.partnerId === currentUserId)) && (
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() =>
