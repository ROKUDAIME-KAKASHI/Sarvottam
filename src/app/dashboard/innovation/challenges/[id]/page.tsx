    ...
    <div className="flex-1 space-y-4 p-8 pt-6">
      <div className="flex items-center space-x-4 mb-4">
        <Button variant="outline" size="icon" asChild>
          <Link href="/dashboard/innovation"><ArrowLeft className="h-4 w-4" /></Link>
        </Button>
        <div>
              ...
              <CardContent className="space-y-4">
                {challenge.pitches.map(pitch => (
                  <div key={pitch.id} className="p-4 border rounded-lg">
                    ...
                    <p className="text-xs text-muted-foreground mb-2">Submitted by: {pitch.submitter.name || pitch.submitter.email}</p>
                    <p className="text-sm">{pitch.description}</p>
                    {pitch.deckUrl && (
                      <a href={pitch.deckUrl} target="_blank" rel="noreferrer" className="text-sm text-blue-500 hover:underline mt-2 block">
                        View Pitch Deck
                      </a>
                    )}
                  </div>
                ))}
                {challenge.pitches.length === 0 && <p className="text-sm text-muted-foreground">No pitches submitted yet.</p>}
              </CardContent>
            </Card>
          )}
        </div>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{challenge.deadline.toLocaleDateString()}</p>
                </div>
              </div>
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{challenge.prize || "Not specified"}</p>
                </div>
              </div>
            </CardContent>
          </Card>
