        ...
        {["FACULTY", "INDUSTRY_PARTNER", "SUPERADMIN"].includes(userRole) && (
          <div className="relative z-10 shrink-0">
            <Link 
              ...
              className={buttonVariants({ className: "rounded-full shadow-lg shadow-primary/20 hover:shadow-primary/40 transition-all", size: "lg" })}
            >
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {jobs.length === 0 ? (
            ...
            jobs.map((job) => {
              ...
              return (
                <Card key={job.id} className="rounded-2xl border-border/50 bg-card/60 backdrop-blur-sm hover:border-primary/30 transition-all hover:shadow-md hover:-translate-y-1 overflow-hidden">
                  ...
                  <CardFooter className="pt-2 flex justify-between items-center bg-muted/20 border-t border-border/50 rounded-b-2xl p-4">
                    <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
                      ...
                      <span className="bg-foreground/10 px-2 py-0.5 rounded text-[10px] ml-1 uppercase">{job.poster.role}</span>
                    </div>
                    {userRole === "STUDENT" && (
                      hasApplied ? (
                        ...
                        <Link href={`/dashboard/placements/${job.id}`} className={buttonVariants({ variant: "default", className: "rounded-full px-6 shadow-sm shadow-primary/20" })}>
                          View & Apply
                        </Link>
                      )
                    )}
          ...
          <Card className="rounded-3xl border-border/50 bg-gradient-to-br from-primary/10 via-card to-card overflow-hidden">
            ...
            <CardContent className="space-y-4">
              {[
                ...
                <div key={i} className="flex items-center gap-3 p-3 rounded-xl hover:bg-muted/50 transition-colors cursor-pointer border border-transparent hover:border-border/50">
