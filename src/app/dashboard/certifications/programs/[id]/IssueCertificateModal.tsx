...
export default function IssueCertificateModal({ programId, tracks, users }: { programId: string, tracks: any[], users: any[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await issueCertificate({ ...formData, programId });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger render={<Button><Plus className="w-4 h-4 mr-2" /> Issue Credential</Button>} />
      <DialogContent>
        <form onSubmit={handleSubmit}>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Select onValueChange={(val: any) => setFormData({ ...formData, trackId: val })}>
                <SelectTrigger>
