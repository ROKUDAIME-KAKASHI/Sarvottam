...
export default function ResearchList({ 
  ...
  projects: { id: string, title: string, description: string, difficultyLevel?: string, duration?: string, type?: string | null, mentor?: { user?: { name: string | null } }, partner?: { companyName: string }, creator?: { name: string | null, email: string | null }, createdAt: Date | string }[], 
  problems?: { id: string, title: string, description: string, type?: string | null, submitter?: { name: string | null, email: string | null, role: string }, facultyApproved: boolean, industryApproved: boolean, adminApproved: boolean, createdAt: Date | string }[],
  currentUserRole?: string
}) {
  const [isPending, startTransition] = useTransition();
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [applyingProject, setApplyingProject] = useState<{ id: string, title: string } | null>(null);
  ...
  const filteredCombined = filterType === "All" 
    ? combined 
          ...
          <Card className="flex flex-col h-full rounded-[2rem] border-border/50 bg-background/60 backdrop-blur-xl hover:shadow-2xl hover:shadow-primary/10 transition-all duration-500 group overflow-hidden relative border-t-primary/10">
            ...
            <CardHeader className="p-8 pb-4 relative z-10">
              <div className="flex justify-between items-start mb-6">
                <Badge variant="secondary" className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${item.category === 'project' ? 'bg-primary/5 text-primary border-primary/10' : 'bg-amber-500/10 text-amber-600 border-amber-500/20'}`}>
            ...
            <CardContent className="p-8 pt-4 flex-1 space-y-6 relative z-10">
              <div className="grid grid-cols-2 gap-4">
                <div suppressHydrationWarning className="flex items-center text-sm font-medium text-muted-foreground">
                  ...
                  {item.category === 'project' ? (item.duration || "Ongoing") : new Date(item.createdAt).toLocaleDateString()}
                </div>
              ...
              {item.category === 'problem' && (
                <div className="flex flex-wrap gap-2 pt-4 border-t border-border/50">
                  ...
                  <Badge variant="outline" className={`text-[10px] uppercase font-bold ${item.adminApproved ? 'bg-blue-500/10 text-blue-600 border-blue-500/20' : 'bg-red-500/10 text-red-600 border-red-500/20'}`}>
                    {item.adminApproved && <CheckCircle2 className="w-3 h-3 mr-1 inline-block" />}
    ...
    {applyingProject && (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
        ...
        >
          ...
          <p className="text-muted-foreground font-medium mb-6">Project: <span className="text-foreground font-bold">{applyingProject.title}</span></p>
          
          <form onSubmit={handleApply} className="space-y-6">
            <div className="space-y-2">
              <label className="text-xs font-black text-muted-foreground uppercase tracking-widest pl-1">Confirm Your Skills</label>
              <textarea 
                required
                value={skills}
            ...
            <div className="flex justify-end gap-3 pt-4">
              <Button type="button" variant="ghost" onClick={() => setApplyingProject(null)} className="rounded-xl h-12 px-6 font-bold">Cancel</Button>
              <Button type="submit" disabled={isPending} className="rounded-xl h-12 px-8 font-black shadow-lg shadow-primary/20">
